...
What the replay contains depends on the backend. A tmux or screen session holds the complete screen itself, and its transcript is mostly earlier repaints of that screen, so a client that connects without `since` receives no transcript frames at all. It gets one replay frame with sequence 0 carrying the *prelude*, the first 4 KiB the multiplexer printed when wmux attached (alternate screen, bracketed paste and keypad modes, terminal queries), which a repaint does not repeat; the replay barrier swallows the answers to those queries as it does for any replay. Then the server asks the multiplexer to repaint, and the repaint arrives as ordinary live output after `replay_end`. tmux is repainted with a SIGWINCH to wmux's own client (locally a signal, remotely `kill -WINCH` over a second channel of the existing SSH connection), which makes it invalidate the terminal and re-send its modes, mouse tracking included; `refresh-client` would only redraw the content. screen is repainted with `screen -X redisplay`. A reconnecting client still catches up on the frames after `since` when they fit in 256 KiB; a larger gap is skipped in favour of a repaint. A direct PTY (`none`) has no such repaint, so it replays its transcript as before; when that replay was cut short, the server shrinks the PTY by one row and restores it so that full-screen programs repaint over the partial state, and shells ignore it.
